			    ...
			    <div class="row">
					...
					<form action="<?php $_PHP_SELF ?>" method="post">
                                            <div class="form-group row">
                                                <label for="example-password-input" class="col-sm-2 col-form-label">Client's RO No</label>
                                                <div class="col-sm-10">
                                                     <select class="form-control" name="ro[]" required>
                                                        <option value="">Select</option>
                                                        <?php
							 include("../connection.php");
							 $sup1 = "select rono,client from ro_led where s=0 and emp_name=(select name from employee_details where id=".$_SESSION['user_id'].")";
	...
	$(document).ready(function() {
		...
		if(x < max_fields){ //max input box allowed
		    ...
		    <?php
							...
							$sup2 = mysqli_query($conn,$sup1);
		?>
		    $(wrapper).append('<div><a href="#" class="remove_field"><center>Remove New RO</center></a><br><div class="form-group row"><label for="example-tel-input" class="col-sm-2 col-form-label">Newspaper RO No</label><div class="col-sm-10"><select class="form-control" name="ro[]" required><option value="">Select</option><?php while($sup3=mysqli_fetch_array($sup2)) { extract($sup3); echo '<option value='.$rono.'>'.$rono.' - '.$client.'</option>'; } ?></select></div></div></div>'); //add input box
		}
	    });
...
<?php
	if(isset($_POST['save']))
	{
		...
		echo "<script type='text/javascript'>
			window.location.href = 'led_sales.php?$ttt';
			</script>";
	}
?>