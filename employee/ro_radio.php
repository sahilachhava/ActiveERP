...
<head>
        ...
        <title>Add Radio RO :: Active ERP</title>
   ...
   <?php include('header.php'); ?>
                                    ...
                                    <div class="page-title-box">
                                        <div class="btn-group float-right">
                                            <ol class="breadcrumb hide-phone p-0 m-0">
                                                ...
                                                <li class="breadcrumb-item active">Add Radio RO</li>
                                            </ol>
                                        </div>
                                        <h4 class="page-title">Add Radio RO</h4>
                                    </div>
                                </div>
                            </div>
			    ...
			    <div class="row">
					...
					<form action="<?php $_PHP_SELF ?>" method="post">
					    ...
					    <div class="form-group row">
                                                <label for="example-tel-input" class="col-sm-2 col-form-label">Radio Station</label>
                                                <div class="col-sm-10">
                                                    <select class="form-control" name="radio[]" required>
                                                        <option value="">Select</option>
						<?php
							 include("../connection.php");
							 $sup1 = "select name from product where type ='Radio Station'"; 
							$sup2 = mysqli_query($conn,$sup1);
							while($sup3=mysqli_fetch_array($sup2))
							{
								extract($sup3);
						...
						?>
                                            ...
                                            <div class="form-group row">
                                                <label for="example-datetime-local-input" class="col-sm-2 col-form-label">Commercial Type</label>
                                                <div class="col-sm-10">
                                                    <input class="form-control" type="text" placeholder="FCT etc" name="ct[]" value="" id="example-datetime-local-input" required>
                                                </div>
                                            </div>
                                            <div class="form-group row">
                                                <label for="example-time-input" class="col-sm-2 col-form-label">Total Duration</label>
                                                <div class="col-sm-10">
                                                    <input class="form-control" type="text" value="" placeholder="Total Duration in Seconds" name="dur[]" id="example-time-input" required>
                                                </div>
                                            </div>
                                            <div class="form-group row">
                                                <label for="example-color-input" class="col-sm-2 col-form-label">Total Spots</label>
                                                <div class="col-sm-10">
                                                    <input class="form-control" type="text" value="" placeholder="Total Spots" name="spot[]" id="example-time-input">
                                                </div>
                                            </div>
					<div class="form-group row">
...
<?php
	if(isset($_POST['rate']))
	{
		...
		if(isset($_POST["radio"]) && is_array($_POST["radio"])){
			$news = implode(",", $_POST["radio"]); 
		}
		$client = $_POST['client'];
		$edi = "";
		if(isset($_POST["sd"]) && is_array($_POST["sd"])){
			$edi = implode(",", $_POST["sd"]); 
		}
		$rdt = "";
		if(isset($_POST["ed"]) && is_array($_POST["ed"])){
			$rdt = implode(",", $_POST["ed"]); 
		}
		$size = "";
		if(isset($_POST["ct"]) && is_array($_POST["ct"])){
			$size = implode(",", $_POST["ct"]); 
		}
		$page = "";
		if(isset($_POST["dur"]) && is_array($_POST["dur"])){
			$page = implode(",", $_POST["dur"]); 
		}
		$print = "";
		if(isset($_POST["spot"]) && is_array($_POST["spot"])){
			$print = implode(",", $_POST["spot"]); 
		}
		$rate = "";
		...
		$sql = "insert into ro_radio(sup,dt,bdt,radio,hsn,client,sd,ed,ct,dur,spot,rate,dis,amt,sgst,cgst,gtotal,emp_name) 
		values('$sup','$dt','$bdt','$news','$hs','$client','$edi','$rdt','$size','$page','$print','$rate','$dis','$amt','$sgst','$cgst','$gtotal','$emp')";
		$result = mysqli_query($conn,$sql);
		$sql2 = "select id from ro_radio where id=(select max(id) from ro_radio)";
		$result2 = mysqli_query($conn,$sql2);
		$row = mysqli_fetch_array($result2);
		$ron = $row[0];
		
		$sql6 = "insert into ro(type,type_id) values('radio','$ron')";
		...
		$sql8 = "update ro_radio set r_no='$inn' where id=".$ron;
		$result8 = mysqli_query($conn,$sql8);
		
		$sql1 = "insert into rcombo(ro_no,ro_dt,sup,emp_name) values('$inn','$dt','$sup','$emp')";
		$result1 = mysqli_query($conn,$sql1);
		$sql3 = "insert into combo(ro_no,ro_dt,sup,emp_name) values('$inn','$dt','$sup','$emp')";
		$result3 = mysqli_query($conn,$sql3);
		echo "<script type='text/javascript'>
			window.location.href = 'rinvoice.php';
			</script>";
	}
?>