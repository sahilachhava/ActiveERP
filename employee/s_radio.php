...
<head>
    ...
    <?php include('header.php'); ?>
			    ...
			    <div class="row">
					...
					<form action="<?php $_PHP_SELF ?>" method="post">
						...
						{
						$s = $_GET['po'];
						$sql1 = "select * from ro_radio where r_no=".$s;
						...
						while($row1=mysqli_fetch_array($result1))
						{
							...
							for($i=0; $i<$le; $i++)
							{
								...
								echo "<input class='form-control' type='text' value='$rad[$i], Spot:  $size1[$i],  Amount: $amt1[$i]' id='example-time-input' disabled> 
								</div>
							    </div>";
								echo "<div class='form-group row'>";
	...
	$('document').ready(function () {
		...
		inputOptions: {
			<?php
				$sl = "select * from ro_radio where s=0 and emp_name=(select name from employee_details where id=".$_SESSION["user_id"].")";
	...
	allowOutsideClick: false,
		...
		window.location.href='s_radio.php?po='+text;
	    }, function (dismiss) {
				if (dismiss === 'cancel') {
				   window.history.back();
				}
...
<?php
	...
	if(isset($_POST['save']))
	{
		...
		$sql = "insert into sales_radio(sup,dt,radio,hsn,client,sd,ed,ct,dur,spot,rate,dis,amt,sgst,cgst,gtotal,ro_no,p_total,profit,emp_name) 
		values('$supp','$dt','$rd','$hs','$cus','$start','$end','$c','$du','$sp','$rate','$dis','$amt','$sgst','$cgst','$gtotal','$ron','$pt','$profit','$emp')";
		$result = mysqli_query($conn,$sql);
		$sql1 = "update ro_radio set s_total= '$amt', profit='$profit', s=1 where r_no=".$ron;
		$result1 = mysqli_query($conn,$sql1);
		$sql1 = "update purchase_radio set s_total= '$amt', profit='$profit' where r_no=".$ron;
		$result1 = mysqli_query($conn,$sql1);
		...
		$sql3 = "update rcombo set sale_in='$inn', sale_dt='$dt', sale_amt='$amt', sale_gst='$tax', sale_total='$gtotal', profit='$profit', gst='$mgst' where ro_no=".$ron;
		...
		echo "<script type='text/javascript'>
			window.location.href = 'radio_invoice.php';
			</script>";
	}
?>