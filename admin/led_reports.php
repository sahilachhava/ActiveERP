    ...
    <?php include('header.php'); ?>
			...
			<label for="example-month-input" class="col-sm-1 col-form-label">Employee</label>
					...
					<label for="example-month-input" class="col-sm-1 col-form-label">RO No</label>
                                                <div class="col-sm-2">
                                                    <select class="form-control tax" name="tax">
                                                        <option value="">Select</option>
						<?php
							$sql2= "select client,rono from sales_led";
							$re1 = mysqli_query($conn,$sql2);
							while($row=mysqli_fetch_array($re1))
							{
								extract($row);
				...
				else
				{
					...
					$ta = $_REQUEST["tax"];
					if($_REQUEST["emp"] != ""){
						$query = "SELECT * FROM `sales_led` WHERE `emp_name` like '$s%'";
					}
					if($_REQUEST["month"] != ""){
						if($query == ""){
							$query = "SELECT * FROM `sales_led` WHERE `dt` BETWEEN '2018/$mon/01' AND '2018/$mon/31'";
						}else{
					...
					if($_REQUEST["tax"] != ""){
						if($query == ""){
							$query = "SELECT * FROM `sales_led` WHERE `rono`='$ta'";
						}else{
							$query .= " AND `rono`='$ta'";
						}
					}
