...
<head>
    ...
    <?php include('header.php'); ?>
			...
			<label for="example-month-input" class="col-sm-1 col-form-label">Employee</label>
					...
					<label for="example-month-input" class="col-sm-1 col-form-label">RO No</label>
                                                <div class="col-sm-2">
                                                    <select class="form-control tax" name="tax">
                                                        <option value="">Select</option>
						<?php
							$sql2= "select client,rono from sales_rail";
							$re1 = mysqli_query($conn,$sql2);
							while($row=mysqli_fetch_array($re1))
							{
								extract($row);
				...
				$query =  "";
				$coun = $coun1 = $total = 0;
				if(!isset($_REQUEST['emp']) or !isset($_REQUEST['tax']) or !isset($_REQUEST['month']))
				{
					$sql2 = "select * from sales_rail";
				...
				else
				{
					...
					$ta = $_REQUEST["tax"];
					if($_REQUEST["emp"] != ""){
						$query = "SELECT * FROM `sales_rail` WHERE `emp_name` like '$s%'";
					}
					if($_REQUEST["month"] != ""){
						if($query == ""){
							$query = "SELECT * FROM `sales_rail` WHERE `dt` BETWEEN '2018/$mon/01' AND '2018/$mon/31'";
						}else{
							$query .= " AND `dt` BETWEEN '2018/$mon/01' AND '2018/$mon/31'";
						}
					}
					if($_REQUEST["tax"] != ""){
						if($query == ""){
							$query = "SELECT * FROM `sales_rail` WHERE `rono`='$ta'";
						}else{
							$query .= " AND `rono`='$ta'";
						}
					}
                                ...
                                <div class="col-12">
                                    <div class="card m-b-30">
                                        <div class="card-body">
                                            ...
                                            <h4 class="mt-0 header-title"><?php if(isset($_REQUEST['emp']))
												{
													$emp = $_REQUEST['emp'];
													if($emp=="")
													{
	...
	<script>
		...
		$("#submit").click(function () {
		    ...
		    var tax = $('.tax');
		    var mn = $('.month');
		    if (emp.val()==='' && tax.val()==='' && mn.val()==='') 
		    {
			    swal({
				title: 'Wrong Action (Value needed)',
				text: 'Please select employee name or invoice or month for reports, this box will be close in 5 seconds',
				timer: 5000
			    }).then(
				function () {
				},
