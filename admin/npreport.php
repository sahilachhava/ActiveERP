			...
			<div class="row">
					...
					<th>Our Profit</th>
						...
						if(isset($_REQUEST['emp']) or isset($_REQUEST['month']))
						{
							...
							$dd = $_REQUEST["month"];
							if($_REQUEST["emp"] != "0"){
								$query = "SELECT * FROM `purchase_newspaper` WHERE `emp_name` like '$s%'";
							}
							if($_REQUEST["month"] != "0"){
								if($query == ""){
									$query = "SELECT * FROM `purchase_newspaper` WHERE `dt` BETWEEN '2018/$dd/01' AND '2018/$dd/31'";
								}else{
						...
						{
							include("../connection.php");
							$sql = "select * from purchase_newspaper";
