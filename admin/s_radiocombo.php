					...
					<?php
						...
						if(isset($_GET))
						{
							...
							for($i=0;$i<$s;$i++)
							{
								...
								$sql1 = "select * from ro_radio where r_no=".$rde[$i];
								$result1 = mysqli_query($conn,$sql1);
								while($row1=mysqli_fetch_array($result1))
								{
									$j =0;
									$rad = explode(",",$row1['radio']);
									$quan1 = explode(",",$row1['dur']);
									$rt = explode(",",$row1['rate']);
									$di = explode(",",$row1['dis']);
									$amt1 = explode(",",$row1['amt']);
									$size1 = explode(",",$row1['spot']);
									$le = count($rad);
									for($k=0; $k<$le; $k++)
									{
										$j++;
										...
										echo "<input class='form-control' type='text' value='$rad[$k], Duration:  $quan1[$k], Rate: $rt[$k], Dis: $di[$k]%,  Amount: $amt1[$k]' id='example-time-input' disabled> 
										</div>
									    </div>";
										echo "<div class='form-group row'>";
										echo "<label for='example-time-input' class='col-sm-2 col-form-label'>Client's Rate</label>";
										echo "<div class='col-sm-10'>";
										echo "<input class='form-control' type='text' value='' placeholder='Rate per second' name='rate[]' id='example-time-input' required> 
										</div>
									    </div>";
									    echo "<div class='form-group row'>";
...
<?php
	if(isset($_POST['save']))
	{
		...
		{
			$ron[$i] = $_GET['ro'.$i];
			$sql1 = "select * from ro_radio where r_no=".$ron[$i];
			$result1 = mysqli_query($conn,$sql1);
		...
		$new4 = "";
			...
			$ron = implode(",", $ron); 
			$pt = implode(",", $pt); 
			//echo $rd."<br>".$start."<br>".$end."<br>".$c."<br>".$du."<br>".$sp."<br>".$ron."<br>".$pt."<br>";
			
		$sql = "insert into sales_radio(sup,dt,radio,hsn,client,sd,ed,ct,dur,spot,rate,dis,man,manch,amt,sgst,cgst,gtotal,ro_no,p_total,profit,emp_name) 
		values('$supp','$dt','$rd','$hs','$cus','$start','$end','$c','$du','$sp','$rate','$dis','$mcrate','$manch','$amt','$sgst','$cgst','$gtotal','$ron','$pt','$profit','$emp')";
		...
		for($i=0;$i<$s;$i++)
		{
			$sql1 = "update ro_radio set s_total= '$sta[$i]', profit='$proft[$i]', s = 1 where r_no=".$_GET['ro'.$i];
			$result1 = mysqli_query($conn,$sql1) or die(mysqli_error($conn));
			
			$sql1 = "update purchase_radio set s_total= '$sta[$i]', profit='$proft[$i]' where ro_no=".$_GET['ro'.$i];
			$result1 = mysqli_query($conn,$sql1) or die(mysqli_error($conn));
		}
		
		$sql4 = "select id from sales_radio where id=(select max(id) from sales_radio)";
		$result4 = mysqli_query($conn,$sql4);
		$row = mysqli_fetch_array($result4);
		$son = $row[0];
		
		$sql6 = "insert into invoice(type,type_id) values('radio','$son')";
		...
		$sql8 = "update sales_radio set in_no='$inn' where id=".$son;
		$result8 = mysqli_query($conn,$sql8);
		
		$poff = explode(",",$profit);
		$pof = array_sum($poff);
		
		$sql9 = "insert into income(dt,pur,amt,des,emp,sale_id,type,in_no,ron) values('$dt','Tax Invoice No - T/0$inn','$pof','Client: $cus,  Release Order No: R/0$ron','$emp','$son','radio','$inn','$ron')";
		...
		for($i=0;$i<$s;$i++)
		{
			$sql3 = "update rcombo set sale_in='$inn', sale_dt='$dt', sale_amt='$sta[$i]', sale_gst='$stgs[$i]', sale_total='$stgt[$i]', profit='$proft[$i]', gst='$mmgs[$i]' where ro_no=".$_GET['ro'.$i];
		...
		echo "<script type='text/javascript'>window.location.href = 'radio_invoice.php';</script>";
	}
?>