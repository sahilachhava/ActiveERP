...
<head>
        ...
        <title>Add LED Sales :: Active ERP</title>
                                    ...
                                    <div class="page-title-box">
                                        <div class="btn-group float-right">
                                            <ol class="breadcrumb hide-phone p-0 m-0">
                                                ...
                                                <li class="breadcrumb-item active">LED Sales</li>
                                            </ol>
                                        </div>
                                        <h4 class="page-title">Add LED Sales</h4>
                                    </div>
                                </div>
                            </div>
					...
					<form action="<?php $_PHP_SELF ?>" method="post">
						...
						if(isset($_GET))
						{
							...
							for($i=0;$i<$s;$i++)
							{
								...
								$sql1 = "select * from ro_led where rono='$rde[$i]'";
								...
								while($row1=mysqli_fetch_array($result1))
								{
									...
									for($k=0; $k<$le; $k++)
									{
										...
										echo "<input class='form-control' type='text' value='$cn[$k], Duration:  $dur1[$k],  Rate: $rt[$k], Dis: $di[$k]%,  Amount: $amt1[$k]' id='example-time-input' disabled> 
										</div>
									    </div>";
										echo "<div class='form-group row'>";
...
<?php
	if(isset($_POST['save']))
	{
		...
		{
			$ron[$i] = $_GET['ro'.$i];
			$sql1 = "select * from ro_led where rono='$ron[$i]'";
			$result1 = mysqli_query($conn,$sql1);
		...
		$sql = "insert into sales_led(dt,hsn,client,gst_type,rono,cn,sd,ed,cd,spd,spot,dur,rate,dis,man,manch,amt,sgst,cgst,igst,gtotal,p_total,profit,emp_name) 
		values('$dt','$hs','$cus','$gsty','$ron','$cn','$sd','$ed','$cd','$spd','$spot','$dur','$rate','$dis','$mcrate','$manch','$amt','$sgst','$cgst','$tax','$gtotal','$pt','$profit','$emp')";
		$result = mysqli_query($conn,$sql) or die(mysqli_error($conn));
			...
			for($i=0;$i<$s;$i++)
			{
				$sql1 = "update ro_led set s_total= '$sta[$i]', profit='$proft[$i]', s=1 where rono=".$_GET['ro'.$i];
				$result1 = mysqli_query($conn,$sql1);
			}
			
			$sql4 = "select id from sales_led where id=(select max(id) from sales_led)";
			$result4 = mysqli_query($conn,$sql4);
			$row = mysqli_fetch_array($result4);
			$son = $row[0];
		
			$sql6 = "insert into invoice(type,type_id) values('led','$son')";
			...
			$sql8 = "update sales_led set in_no='$inn' where id=".$son;
			$result8 = mysqli_query($conn,$sql8);
			
			$pof = array_sum(explode(",",$manch)) + array_sum(explode(",",$amt));
			
			$sql9 = "insert into income(dt,pur,amt,des,emp,sale_id,type,in_no,ron) values('$dt','Tax Invoice No - T/0$inn','$pof','Client: $cus,  Release Order No: $ron, Campaign Name: $cn','$emp','$son','led','$inn','$ron')";
			$result9 = mysqli_query($conn,$sql9) or die(mysqli_error($conn));
		
			echo "<script type='text/javascript'>window.location.href = 'led_invoice.php';</script>";
	}
?>