			...
			<div class="row">
					...
					<th>Our Profit</th>
						...
						if(isset($_REQUEST['emp']) or isset($_REQUEST['month']))
						{
							...
							$dd = $_REQUEST["month"];
							if($_REQUEST["emp"] != ""){
								$query = "SELECT * FROM `sales_newspaper` WHERE `emp_name` like '$s%'";
							}
							if($_REQUEST["month"] != "0"){
								if($query == ""){
									$query = "SELECT * FROM `sales_newspaper` WHERE `dt` BETWEEN '2018/$dd/01' AND '2018/$dd/31'";
								}else{
						...
						{
							include("../connection.php");
							$sql = "select * from sales_newspaper";
