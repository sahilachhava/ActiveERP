					  ...
					  <?php
						...
						if(isset($_GET))
						{
							...
							for($i=0;$i<$s;$i++)
							{
								...
								$sql1 = "select * from ro_hoarding where r_no=".$rde[$i];
								$result1 = mysqli_query($conn,$sql1);
								while($row1=mysqli_fetch_array($result1))
								{
									$j = 0;
									$new = explode(",",$row1['quan']);
									$rt = explode(",",$row1['rate']);
									$prt = explode(",",$row1['prate']);
									$mrt = explode(",",$row1['mrate']);
									$di = explode(",",$row1['dis']);
									$lc = explode("~",$row1['loc']);
									$amt1 = explode(",",$row1['amt']);
									$pamt1 = explode(",",$row1['pamt']);
									$mamt1 = explode(",",$row1['mamt']);
									...
									for($k=0; $k<$le; $k++)
									{
										...
										echo "<input class='form-control' type='text' value='$lc[$k], Size:  $size1[$k], Rate: $rt[$k], P-Rate: $prt[$k], M-Rate: $mrt[$k], Dis: $di[$k]%,  Amount: $amt1[$k]' id='example-time-input' disabled> 
										</div>
									    </div>";
										echo "<div class='form-group row'>";
...
<?php
	if(isset($_POST['save']))
	{
		...
		$ron = $supp = $news = $cus = $edition = $rel = $sze = $pg = $pri = $qu = $hs = $pt = $ttt = $ttt1 = $manch = "";
		...
		{
			$ron[$i] = $_GET['ro'.$i];
			$sql1 = "select * from ro_hoarding where r_no=".$ron[$i];
			$result1 = mysqli_query($conn,$sql1);
			while($row=mysqli_fetch_array($result1))
			{
				...
				$ptp[$i] = $row['pamt'];
				$ptm[$i] = $row['mamt'];
				$pt[$i] = $row['amt'] + $row['pamt'] + $row['mamt'];
				$ttt[$i] = $row['cgst'];
				$ttt1[$i] = $row['sgst'];
				$emp = $row['emp_name'];
			}
			...
			if($i==0)
			{
				...
				for(; $j<$le; $j++)
				{
					$val = explode("x",$sze1[$l]); 
					$va1 = $val[0];
					$va2 = $val[1]; 
					$sqft[$l] = $va1 * $va2;
			
					$pamt[$j] = $prate1[$j] * $sqft[$l];
					$mamt[$j] = $mrate1[$j] * $sqft[$l];
					
					$sub[$j] = $rate1[$k] * $quu1[$l];
					$discount[$j] = $sub[$j] * $dis1[$k] / 100;
					$amt[$j] = $sub[$j] - $discount[$j];
					
					$ffamt[$j] = $amt[$j] + $pamt[$j] + $mamt[$j];
					$manch[$j] = $ffamt[$j] * $mcrate1[$k] / 100;
					$famt[$j] = $amt[$j] + $pamt[$j] + $mamt[$j] + $manch[$j];
					
					if($ttt4=="")
					{
						$d=0;
						$d = $amt[$j] + $pamt[$j] + $mamt[$j] + $manch[$j];
						$ttt4 = $d;
					}
					else
					{
					...
					$dps[$j] = $amt1[$l] + $pamt1[$l] + $mamt1[$l];
					$profit[$j] = $famt[$j] - $dps[$j];
					if($ttt2=="")
					{
						$d=0;
						$d = $famt[$j] - $dps[$j];
		...
		$sql = "insert into sales_hoarding(sup,dt,hsn,client,loc,size,sd,ed,quan,rate,prate,mrate,dis,man,manch,amt,pamt,mamt,sgst,cgst,gtotal,ro_no,p_total,profit,emp_name) 
		values('$supp','$dt','$hs','$cus','$lo','$sze','$start','$end','$qu','$rate','$prate','$mrate','$dis','$mcrate','$manch','$amt','$pamt','$mamt','$sgst','$cgst','$gtotal','$ron','$pt','$profit','$emp')";
		...
		for($i=0;$i<$s;$i++)
		{
			$sql1 = "update ro_hoarding set s_total= '$sta[$i]', profit='$proft[$i]', s=1 where r_no=".$_GET['ro'.$i];
			$result1 = mysqli_query($conn,$sql1) or die(mysqli_error($conn));
			
			$sql1 = "update purchase_hoarding set s_total= '$sta[$i]', profit='$proft[$i]' where ro_no=".$_GET['ro'.$i];
			$result1 = mysqli_query($conn,$sql1) or die(mysqli_error($conn));
		}
		...
		$sql9 = "insert into income(dt,pur,amt,des,emp,sale_id,type,in_no,ron) values('$dt','Tax Invoice No - T/0$inn','$pof','Client: $cus,  Release Order No: R/0$ron','$emp','$son','hoarding','$inn','$ron')";
		...
		for($i=0;$i<$s;$i++)
		{
			$sql3 = "update hcombo set sale_in='$inn', sale_dt='$dt', sale_amt='$sta[$i]', sale_gst='$stgs[$i]', sale_total='$stgt[$i]', profit='$proft[$i]', gst='$mmgs[$i]' where ro_no=".$_GET['ro'.$i];
		...
		echo "<script type='text/javascript'>window.location.href = 'hoarding_invoice.php';</script>";
	}
?>